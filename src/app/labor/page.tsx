...
import { Search, Plus, Edit2, Trash2, UserCog, DollarSign, FileText } from 'lucide-react';
...
export default function LaborPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Erro ao criar função de mão de obra:', error);
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Gerencie os custos por hora de cada função.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-muted/30 border-b border-border">
                <th className="px-6 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Função</th>
                <th className="px-6 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Descrição</th>
                <th className="px-6 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Custo por Hora</th>
            ...
            <tbody className="divide-y divide-border/50">
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-slate-500">Nenhuma função cadastrada.</td>
                </tr>
              ) : (
                labor.map((l) => (
                  <tr key={l._id} className="hover:bg-accent/10 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        <div className="w-8 h-8 rounded bg-yellow-500/10 flex items-center justify-center mr-3 text-yellow-500">
                    ...
                    <td className="px-6 py-4 font-medium text-indigo-400">
                      {formatCurrency(l.costPerHour)}/h
                    </td>
      ...
      <Modal 
        ...
        title="Nova Função de Mão de Obra"
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input
            label="Nome da Função"
            placeholder="Ex: Soldador Senior"
            value={formData.name}
