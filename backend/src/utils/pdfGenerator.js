...
exports.generateBudgetPDF = void 0;
const PDFDocument = require('pdfkit');
const Budget_1 = __importDefault(require("../models/Budget"));
const Tenant_1 = __importDefault(require("../models/Tenant"));
const logger_1 = __importDefault(require("./logger"));
const generateBudgetPDF = (budgetId) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b, _c;
    logger_1.default.info('Iniciando geração de PDF', { budgetId });
    try {
        ...
        logger_1.default.info('Budget consultado', { budgetId, found: !!budget });
        if (!budget) {
            throw new Error('Orçamento não encontrado');
        }
        logger_1.default.debug('Budget data', {
            number: budget.number,
            title: budget.title,
            clientId: budget.clientId,
            materials: (_a = budget.materials) === null || _a === void 0 ? void 0 : _a.length,
            labor: (_b = budget.labor) === null || _b === void 0 ? void 0 : _b.length,
            machines: (_c = budget.machines) === null || _c === void 0 ? void 0 : _c.length,
            tenantId: budget.tenantId
        });
        const tenant = yield Tenant_1.default.findById(budget.tenantId);
        logger_1.default.info('Tenant consultado', { tenantId: budget.tenantId, found: !!tenant });
        if (!tenant) {
            throw new Error('Empresa não encontrada');
        }
        logger_1.default.debug('Tenant data', {
            name: tenant.name,
            email: tenant.email
        });
        logger_1.default.info('Iniciando criação do documento PDF', { budgetId });
        ...
        return new Promise((resolve, reject) => {
            doc.on('end', () => {
                logger_1.default.info('PDF finalizado', { budgetId, size: chunks.length });
                resolve(Buffer.concat(chunks));
            });
            doc.on('error', (error) => {
                logger_1.default.error('Erro na geração do PDF', { budgetId, error: error.message });
                reject(error);
            });
            // Header
            ...
            logger_1.default.debug('PDF - materiais do orçamento', {
                total: allBudgetItems.length,
                items: allBudgetItems.map((item) => {
                    var _a, _b, _c, _d, _e, _f;
                    return ({
                        name: (_a = item.materialId) === null || _a === void 0 ? void 0 : _a.name,
                        itemType: (_b = item.materialId) === null || _b === void 0 ? void 0 : _b.itemType,
                        isComposite: (_c = item.materialId) === null || _c === void 0 ? void 0 : _c.isComposite,
                        componentsCount: (_f = (_e = (_d = item.materialId) === null || _d === void 0 ? void 0 : _d.components) === null || _e === void 0 ? void 0 : _e.length) !== null && _f !== void 0 ? _f : 0,
                    });
                })
            });
            const materialItems = allBudgetItems.filter((item) => {
                const mat = item.materialId;
                return !mat || mat.itemType !== 'component';
            });
            const componentItems = allBudgetItems.filter((item) => {
                const mat = item.materialId;
                return mat && mat.itemType === 'component';
            });
            ...
            if (materialItems.length > 0) {
                ...
                materialItems.forEach((item, index) => {
                    ...
                    const weight = (material === null || material === void 0 ? void 0 : material.weight) || 0;
                    const unit = (material === null || material === void 0 ? void 0 : material.weightUnit) || 'kg';
                    ...
                    doc.text((material === null || material === void 0 ? void 0 : material.name) || '-', descriptionX, currentY, { width: 210 });
                ...
                budget.totalWeightCalculation = totalWeightKg;
                doc.y = currentY + 10;
            }
            // Composite product breakdown: sub-components of composite materials
            const compositeItems = materialItems.filter((item) => {
                const mat = item.materialId;
            ...
            if (compositeItems.length > 0) {
                ...
                doc.fillColor('black');
                doc.moveDown();
                compositeItems.forEach((budgetItem) => {
                    const mat = budgetItem.materialId;
            ...
            if (componentItems.length > 0) {
                ...
                componentItems.forEach((item, index) => {
                    var _a, _b, _c, _d;
                    const c = item.materialId;
                    if (!c)
                        return;
                    // Build specifications string
                    const specs = [];
                    if (c.size)
                        specs.push(`Tam: ${c.size}`);
                    if (c.diameter)
                        specs.push(`Ø${c.diameter}${c.diameterUnit || 'mm'}`);
                    if ((_a = c.dimensions) === null || _a === void 0 ? void 0 : _a.width)
                        specs.push(`Larg: ${c.dimensions.width}${c.dimensions.widthUnit || 'mm'}`);
                    const specsText = specs.join(', ');
                    // Dimensions string
                    const dims = [];
                    if (c.diameter)
                        dims.push(`Diâmetro: ${c.diameter}${c.diameterUnit || 'mm'}`);
                    if ((_b = c.dimensions) === null || _b === void 0 ? void 0 : _b.width)
                        dims.push(`Largura: ${c.dimensions.width}${c.dimensions.widthUnit || 'mm'}`);
                    if ((_c = c.dimensions) === null || _c === void 0 ? void 0 : _c.length)
                        dims.push(`Comprimento: ${c.dimensions.length}${c.dimensions.lengthUnit || 'mm'}`);
                    if ((_d = c.dimensions) === null || _d === void 0 ? void 0 : _d.height)
                        dims.push(`Altura: ${c.dimensions.height}${c.dimensions.heightUnit || 'mm'}`);
            ...
            if (budget.labor && budget.labor.length > 0) {
                ...
                let currentY = tableTop + 25;
                budget.labor.forEach((item, index) => {
            ...
            if (budget.machines && budget.machines.length > 0) {
                ...
                doc.moveTo(itemX, tableTop + 15).lineTo(545, tableTop + 15).stroke();
                let currentY = tableTop + 25;
                budget.machines.forEach((item, index) => {
            ...
            doc.moveDown();
            const summaryX = 350;
            const totalWeight = budget.totalWeightCalculation || 0;
            if (totalWeight > 0) {
            ...
            logger_1.default.info('Finalizando documento PDF', { budgetId });
            doc.end();
        });
    }
    catch (error) {
