...
export const createCheckoutSession = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      logger.warn('Falha ao persistir checkout internamente; retornando sessao do Mercado Pago mesmo assim', {
        orderId,
...
export const createPixPayment = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      logger.warn('Falha ao persistir pagamento PIX internamente; retornando dados do Mercado Pago mesmo assim', {
        orderId,
        mercadoPagoPaymentId: payment.id?.toString(),
        error: persistenceError?.message || persistenceError,
      });
...
export const getPaymentStatus = async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({ status: 'pending', id: req.params.paymentId });
  }
};
