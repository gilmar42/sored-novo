...
BudgetSchema.pre('validate', function(next) {
  const materialsCost = this.materials.reduce((total: number, item: IBudgetMaterial) => total + item.totalCost, 0);
  const laborCost = this.labor.reduce((total: number, item: IBudgetLabor) => total + item.totalCost, 0);
  const machinesCost = this.machines.reduce((total: number, item: IBudgetMachine) => total + item.totalCost, 0);
